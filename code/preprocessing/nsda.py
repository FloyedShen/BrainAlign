...
class NSDAccess(object):
    ...
    def read_betas(self, subject, session_index, trial_index=[], data_type='betas_fithrf_GLMdenoise_RR',
                   data_format='fsaverage', mask=None):
    ...
    def list_atlases(self, subject, data_format='fsaverage', abs_paths=False):
        ...
        if abs_paths:
            ...
            return np.unique(
                [op.split(f)[1].replace('lh.', '').replace('rh.', '').replace('.mgz', '').replace('.nii.gz', '') for f
                 in atlas_files])
